 ...
 * [二分搜索]算法技巧：
 * 使用寻找一个数（基本的二分搜索）
 * 做指针设置0，右指针设置nums长度-1.
 * while 循环 left <= right ，因为右指针是长度-1。
 * 找中位，防止左右指针太大直接相加导致溢出。
 * 中位数 nums[mid] 对比目标 target 等于？ 小于？ 大于？
 * 等于即：找到目标值停止搜索
 * 这里注意：由于搜索区间两端都闭 [left,right] ，发现索引 mid不是要找的 target 时， mid 已经搜索过，应该从搜索区间中去除。
 ...
 * left==right 区间为 [right,right] [2,2] -> while 循环终止时[2,2]里的2被漏掉了，索引 2 没有被搜索，直接返回 -1 是错误的。
 ...
 * 此算法返回索引 2 ；但无法得到target 的左侧边界（索引 1）和右侧边界（索引 3），此算法无法处理，如果向左、右线性搜索，难以保证二分查找对数级的复杂度。
